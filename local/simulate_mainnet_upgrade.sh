...
export START_VERSION="v25.0.0"
export FORK_TAG="v25.0.0"
export UPGRADE_NAME=v25.1.0
# ********** <COSMOS HUB **********

export NODE_HOME="$(pwd)/.gaia"
export NODE_MONIKER=upgrade-test
export BINARY=gaiad
export UPGRADE_VERSION=$UPGRADE_NAME
...
curl -OL https://golang.org/dl/go1.24.3.linux-amd64.tar.gz
...
mv genesis.cosmoshub-4.json $NODE_HOME/config/genesis.json
...
tmux new-session -d -s cosmovisor "$HOME/go/bin/cosmovisor run start --home $NODE_HOME | tee $NODE_HOME/cosmovisor.log"
...
current_block=$($BINARY status --home $NODE_HOME | jq -r '.sync_info.latest_block_height')
upgrade_height=$(($current_block+50))
